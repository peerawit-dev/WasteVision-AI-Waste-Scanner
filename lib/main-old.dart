...
class _WasteScannerScreenState extends State<WasteScannerScreen> {
        ...
        if (useOnDeviceMl) {
          ...
          if (labels.isNotEmpty) {
            ...
            for (var label in labels) {
              ...
              final scoreText = "${(label.confidence * 100).toInt()}%";
  ...
  void _analyzeAIMultiple(Map<String, dynamic> json) {
    ...
    final Set<String> usedBinTypes = {}; // เก็บประเภทถังเพื่อหลีกเลี่ยงการซ้ำกัน
    aiResults = [];
  ...
  Map<String, dynamic> _classifyWaste(
    ...
    if (textAndLogo.contains(RegExp(r'namthip|minere|aura|crystal|purra|water'))) {
      return {
        'title': 'PET Bottle',
        'detail': 'ทิ้งถังเหลือง (รีไซเคิล)',
        'score': scoreText,
    ...
    if (textAndLogo
        .contains(RegExp(r'coca-cola|coke|cola|pepsi|est|fanta|sprite'))) {
      return {
        'title': 'Beverage Can',
        'detail': 'ทิ้งถังเหลือง (รีไซเคิล)',
        'score': scoreText,
    ...
    if (textAndLogo.contains(RegExp(r'lay|tasto|snack|chip|crisp|doritos'))) {
      return {
        'title': 'Snack Bag',
        'detail': 'ทิ้งถังน้ำเงิน (ขยะทั่วไป)',
        'score': scoreText,
    ...
    if (textAndLogo.contains(RegExp(r'tissue|napkin|wipe'))) {
      return {
        'title': 'Tissue',
        'detail': 'ทิ้งถังน้ำเงิน (ขยะทั่วไป)',
        'score': scoreText,
    ...
    // เช็ค Label (รูปร่าง)
    if (labelLower.contains(RegExp(r'Bottle|Plastic|Glass|Metal|Can|Tin'))) {
      return {
        'title': label,
        'detail': 'ทิ้งถังเหลือง (รีไซเคิล)',
        'score': scoreText,
    ...
    if (labelLower.contains(RegExp(r'Food|Fruit|Vegetable|Bread'))) {
      return {
        'title': label,
        'detail': 'ทิ้งถังเขียว (ขยะเปียก)',
        'score': scoreText,
    ...
    if (labelLower.contains(RegExp(r'Battery|Spray|Insecticide|Chemical'))) {
      return {
        'title': 'Hazardous',
  ...
  // ไปยังผลลัพธ์ถัดไป
  void _nextResult() {
    if (currentResultIndex < aiResults.length - 1) {
      _displayAIResult(currentResultIndex + 1);
    }
  }

  // ไปยังผลลัพธ์ก่อนหน้า
  void _prevResult() {
  ...
  Widget _buildResultCard() {
    bool hasMultipleResults = aiResults.length > 1;
    return Positioned(
      bottom: 180,
      left: 20,
      right: 20,
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              Row(
                children: [
                  ...
                  if (hasMultipleResults)
                    Text(
                      '${currentResultIndex + 1}/${aiResults.length}',
              ...
              if (hasMultipleResults) ...[const SizedBox(height: 15)],
              if (hasMultipleResults)
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                  children: [
                    ElevatedButton.icon(
                      onPressed: currentResultIndex > 0 ? _prevResult : null,
                    ...
                    ElevatedButton.icon(
                      onPressed:
                          currentResultIndex < aiResults.length - 1
                              ? _nextResult
                              : null,
